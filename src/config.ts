...
export async function loadPluginOptions(options: unknown): Promise<PluginOptions> {
  let fileOptions: unknown = {};
  try {
    const file = Bun.file(translationConfigPath());
    if (await file.exists()) fileOptions = await file.json();
  } catch (error) {
    console.warn("Could not read translate.json; using plugin options", String(error));
  }

  const parsedFileOptions = pluginOptionsSchema.safeParse(fileOptions);
  if (!parsedFileOptions.success) {
    console.warn("Invalid translate.json; using plugin options", parsedFileOptions.error.issues);
    fileOptions = {};
  } else {
    fileOptions = parsedFileOptions.data;
  }

  const parsedFileRecord = z.record(z.string(), z.unknown()).safeParse(fileOptions);
  const parsedInlineRecord = z.record(z.string(), z.unknown()).safeParse(options ?? {});
  const mergedOptions = {
  ...
  console.warn("Invalid auto-translation options; using defaults", parsedOptions.error.issues);
