...
import { createTranslationConfig, getConfigDirectory, registerPlugin } from "../src/installer";

test("resolves OpenCode config directory with environment precedence", () => {
  expect(getConfigDirectory({ OPENCODE_CONFIG_DIR: "/custom", XDG_CONFIG_HOME: "/xdg" })).toBe("/custom");
...
test("registers server and TUI plugins idempotently", () => {
  ...
  expect(JSON.parse(registered)).toEqual({ plugin: ["other", "opencode-auto-translate@latest"] });
  expect(registerPlugin(registered, "opencode.json")).toBe(registered);
  expect(registerPlugin('{"plugin":[["opencode-auto-translate@latest",{}]]}', "tui.json")).toContain(
...
test("creates the guided translation configuration", () => {
  expect(JSON.parse(createTranslationConfig({
    enabled: true,
    lang: "Spanish",
    model: "openai/model",
