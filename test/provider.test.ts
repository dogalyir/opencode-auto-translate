...
test.serial(
  "provider transport logs malformed metadata and fails open",
  async () => {
...
test.serial(
  "provider transport logs missing endpoints and fails open",
  async () => {
    const providerData = {
...
test.serial(
  "provider transport logs HTTP failures and network errors",
  async () => {
    const providerData = {
      all: [
        {
    ...
    expect(
      await failed.translator("hola", model, "to-english"),
    ).toBeUndefined();
    ...
    expect(
      await network.translator("hola", model, "to-english"),
    ).toBeUndefined();
