...
function getPluginFactory(): (...args: never[]) => unknown {
  const pluginDescriptor = Object.getOwnPropertyDescriptor(
    serverModule,
    "default",
  );
  ...
  if (typeof createPlugin !== "function")
    throw new Error("Missing plugin factory");
  return createPlugin;
}
  ...
  const fetchMock = Object.assign(
    async () => {
      ...
      return new Response(
        JSON.stringify({
          choices: [
            { message: { content: typeof text === "string" ? text : null } },
          ],
        }),
        { status: 200 },
      );
...
test("display modes preserve English context", () => {
  ...
  expect(
    displayTranslation("Hello", "Hola", "show original + translation"),
  ).toBe(
    "Hello\n\n----------------------------------------\n[Translation]\nHola",
  );
});
...
test("plugin options provide strict defaults and accept model display settings", () => {
  ...
  expect(
    pluginOptionsSchema.parse({ small_model: "openai/legacy" }),
  ).not.toHaveProperty("small_model");
});
...
test.serial("plugin skips all hooks for excluded agents", async () => {
  ...
  const messageTransform = Reflect.get(
    plugin,
    "experimental.chat.messages.transform",
  );
  const systemTransform = Reflect.get(
    plugin,
  ...
  await Reflect.apply(systemTransform, plugin, [
...
test("translation hooks fail open when configuration response is malformed", async () => {
  ...
  if (typeof transform !== "function")
    throw new Error("Missing message transform");
  const output = {
    messages: [
      {
